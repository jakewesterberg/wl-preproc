...
six-parameter map (design spec section 5). Caching ~38 MB per session of
...
(~2.5 s on a real 1.18M-row recording); the affine transform over those
...
def gaze_trace(path: Path, eye: str, map_: AffineMap) -> np.ndarray:
    """Degrees of visual angle for one eye: `map_` applied to its Purkinje vector."""
    return apply_affine(map_, purkinje_vector(path, eye))
